...
class OrderController extends AllowController {
    ...
    public function index(){
        ...
        if(!empty($_GET['store_name'])){
            $map1['user_store_name'] = array('like',"%{$_GET['store_name']}%");
            $userinfo2 = $user->where($map1)->find();
            $where1['store_id'] = $userinfo2['id'];
            // dump($where1);exit;
        }
        if(!empty($_GET['order_name'])){
            $map['user_name'] = array('like',"%{$_GET['order_name']}%");
            $userinfo1 = $user->where($map)->find();
            $where1['user_id'] = $userinfo1['id'];
            // dump($where1);exit;
        }
        if(!empty($_GET['order_num'])){
            $where1['order_num'] = array('like',"%{$_GET['order_num']}%");
            // dump($where1);exit;
        } 
        ...
        $p=getpage($count,10);
        $allorder = $order->where($where1)->limit($p->firstRow, $p->listRows)->select();
        ...
        foreach($allorder as $k=>$v){
            ...
            //商户名
            $where3['id'] = $v['store_id'];
            $storeinfo = $user->where($where3)->field('user_store_name,user_store_percent')->find();
            $allorder[$k]['user_store_name'] = $storeinfo['user_store_name'];
            //收获信息
            ...
            $allorder[$k]['real_price'] = $v['order_total']*(100-$storeinfo['user_store_percent'])/100;
            // dump($v);
            // dump($storeinfo);
            // dump($allorder[$k]);exit;
        }
        //加载模板
    ...
    public function doaddwuliu(){
        ...
        if($_POST['selectAge']){
            ...
            $date['order_wuliucompany'] = I('post.selectAge','','strip_tags');
        }else{
            //验证规则  
            $rules  = array(
              array('order_wuliunum','require','订单号不能为空!'), 
