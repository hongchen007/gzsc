...
class AdsserveController extends AllowController{
    ...
    public function index(){
        ...
        foreach($UserInfo as $k=>$v){
            ...
            $UserInfo[$k]['user_grade']      = $userinfo1['user_grade'];
            $UserInfo[$k]['user_name']       = $userinfo1['user_name'];
    ...
    public function stop(){
        ...
        //接受值
        $data['id'] = $_GET['id'];
        $data['adsserve_state'] = 2;
        //更新
        if($adsserve->save($data)){
            $this->success('禁用成功',U('Adsserve/index'));
        }else{
            $this->error('禁用失败');
        }
    }
    ...
    public function start(){
        ...
        //更新
        if($adsserve->save($data)){
            $this->success('启用成功',U('Adsserve/index'));
        }else{
            $this->error('启用失败');
        }
    }
    ...
    public function pass(){
        ...
        //更新
        if($adsserve->save($data)){
            $this->success('审核成功',U('Adsserve/index'));
        }else{
            $this->error('审核失败');
        }
    }
