...
class ActivityController extends AllowController{
    ...
    public function insert(){
        ...
        //判断是否已经折扣
        if($good->where($map1)->field('good_attr_discount')->find()){
    ...
    public function index(){
        ...
        //查询数据
        $goodinfo = $good->where($where1)->field('id,user_id,good_name,good_attr_discount')->select();
        ...
        foreach($goodinfo as $k=>$v){
            $map['id'] = $v['user_id'];
            $storeinfo = $user->where($map)->field('user_store_name')->find();
            $goodinfo[$k]['user_store_name'] = $storeinfo['user_store_name'];
        }
        //总长
