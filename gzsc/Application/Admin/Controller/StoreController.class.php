...
//======================================================商铺商品========================================================
    ...
    public function good(){
        ...
        if(!empty($_GET['user_store_name'])){
            $map11['user_store_name'] =array('like',"%{$_GET['user_store_name']}%");
            $userinfo1 = $user->where($map11)->field('id')->find();
            $where['user_id'] = $userinfo1['id'];
        }
        ...
        $p=getpage($count,10);
        $info=$good->where($where)->order('id')->limit($p->firstRow, $p->listRows)->select();
        foreach($info as $k=>$v){
            $a = $cate->field('cate_name')->find($v['cate_id']);
            $info[$k]['cate_name'] = $a['cate_name'];
            $map1['id'] = $v['user_id'];
            $userinfo = $user->where($map1)->field('user_store_name')->find();
...
//=========================================商铺商品附加信息============================================
    ...
    public function doaddattr(){
        ...
        $rules  = array( 
          ...
          array('good_num','require','商品价格不能为空！'),  
