...
<!--        <form class="searchform" action="index.html" method="post">
     ...
     <div class="clearfix" style="margin-left:10px;"> 
      <div class="btn-group"> 
       <button id="editable-sample_new" class="btn btn-primary"> 后台 &gt; 修改<?php echo ($info['good_name']); ?>信息 </button> 
      </div>  
     </div> 

  <div class="panel-body"> 
   <div class="form"> 
    <form class="cmxform form-horizontal adminex-form" id="signupForm" method="post" enctype="multipart/form-data" action="<?php echo U('Store/doeditgood');?>" novalidate="novalidate"> 

    <div class="form-group "> 
      <label for="QQ" class="control-label col-lg-2">修改商品名称</label> 
      <div class="col-lg-10"> 
       <input class="form-control" id="good_name" name="good_name" type="text" value="<?php echo ($info['good_name']); ?>" /> 
       <input class="form-control" id="id" name="id" type="hidden" value="<?php echo ($info['id']); ?>" /> 
      </div> 
     </div> 

    <div class="form-group "> 
      <label for="QQ" class="control-label col-lg-2">修改商品类别</label> 
      <div class="col-lg-10"> 
        <select class="form-control m-bot15" name="good_cate">
          <?php if(is_array($list)): foreach($list as $key=>$row): ?><option value="<?php echo ($row['id']); ?>" <?php if($info['cate_id'] == $row['id'] ): ?>selected<?php endif; ?>><?php echo ($row['cate_name']); ?></option><?php endforeach; endif; ?>
        </select>
      </div> 
     </div>
...
<!--     <div class="form-group "> 
      ...
      <div class="col-lg-10"> 
        <select class="form-control m-bot15" name="good_state">
           <option value="3" <?php if($info['good_state'] == 3): ?>selected<?php endif; ?>>在售</option> 
           <option value="1" <?php if($info['good_state'] == 1): ?>selected<?php endif; ?>>下架</option> 
           <option value="2" <?php if($info['good_state'] == 2): ?>selected<?php endif; ?>>新商品</option> 
        </select>
      </div> 
     </div> -->
