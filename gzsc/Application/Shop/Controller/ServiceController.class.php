...
//================================================二手车信息secondhandcar================================================================
    ...
    public function secondhandcar(){
        ...
        //查询商品数据数据
        $count=$second_hand_car->where($where)->where($map1)->count();
        $p=getpage($count,10);
        $info = $second_hand_car->where($where)->where($map1)->limit($p->firstRow, $p->listRows)->select();
        foreach($info as $k=>$v){
            //判断是否过期
...
//================================================房屋出租houseforrent================================================================
    ...
    public function houseforrent(){
        ...
        //查询商品数据数据
        $count = $house_for_rent->where($where)->where($map1)->count();
        $p=getpage($count,10);
        $info  = $house_for_rent->where($where)->where($map1)->limit($p->firstRow, $p->listRows)->select();
        foreach($info as $k=>$v){
            //判断是否过期
            if($v['valid_time'] > date('Y-m-d H:i:s')){
                $info[$k]['time'] = "正常";
...
//================================================货车出租truckrend================================================================
    ...
    public function truckrend(){
        ...
        //查询商品数据数据
        $count = $truck_rend->where($where)->where($map1)->count();
        $p=getpage($count,10);
        $info  = $truck_rend->where($where)->where($map1)->limit($p->firstRow, $p->listRows)->select();
        foreach($info as $k=>$v){
            //判断是否过期
            if($v['valid_time'] > date('Y-m-d H:i:s')){
                $info[$k]['time'] = "正常";
...
//================================================家居装修homedecoration================================================================
    ...
    public function homedecoration(){
        ...
        //查询商品数据数据
        $count = $home_decoration->where($where)->where($map1)->count();
        $p=getpage($count,10);
        $info  = $home_decoration->where($where)->where($map1)->limit($p->firstRow, $p->listRows)->select();
        foreach($info as $k=>$v){
            //判断是否过期
            if($v['valid_time'] > date('Y-m-d H:i:s')){
                $info[$k]['time'] = "正常";
...
//================================================家政服务housekeeping================================================================
    ...
    public function housekeeping(){
        ...
        //查询商品数据数据
        $count = $housekeeping->where($where)->where($map1)->count();
        $p=getpage($count,10);
        $info  = $housekeeping->where($where)->where($map1)->limit($p->firstRow, $p->listRows)->select();
        foreach($info as $k=>$v){
            //判断是否过期
            if($v['valid_time'] > date('Y-m-d H:i:s')){
                $info[$k]['time'] = "正常";
...
//================================================维修服务weixiuservice================================================================
    ...
    public function weixiuservice(){
        ...
        //查询商品数据数据
        $count = $weixiu_service->where($where)->where($map1)->count();
        $p=getpage($count,10);
        $info  = $weixiu_service->where($where)->where($map1)->limit($p->firstRow, $p->listRows)->select();
        foreach($info as $k=>$v){
            //判断是否过期
            if($v['valid_time'] > date('Y-m-d H:i:s')){
                $info[$k]['time'] = "正常";
...
//================================================医疗medialservice================================================================
    ...
    public function medialservice(){
        ...
        //查询商品数据数据
        $count = $medial_service->where($where)->where($map1)->count();
        $p=getpage($count,10);
        $info  = $medial_service->where($where)->where($map1)->limit($p->firstRow, $p->listRows)->select();
        foreach($info as $k=>$v){
            //判断是否过期
            if($v['valid_time'] > date('Y-m-d H:i:s')){
                $info[$k]['time'] = "正常";
...
//================================================招聘信息inforservice================================================================
    ...
    public function inforservice(){
        ...
        //查询商品数据数据
        $count = $information->where($where)->where($map1)->count();
        $p=getpage($count,10);
        $info  = $information->where($where)->where($map1)->limit($p->firstRow, $p->listRows)->select();
        foreach($info as $k=>$v){
            //判断是否过期
            if($v['valid_time'] > date('Y-m-d H:i:s')){
                $info[$k]['time'] = "正常";
...
//================================================公司工厂company================================================================
    ...
    public function company(){
        ...
        //查询商品数据数据
        $count = $company->where($where)->where($map1)->count();
        $p=getpage($count,10);
        $info  = $company->where($where)->where($map1)->limit($p->firstRow, $p->listRows)->select();
        foreach($info as $k=>$v){
            //判断是否过期
            if($v['valid_time'] > date('Y-m-d H:i:s')){
                $info[$k]['time'] = "正常";
    ...
    public function companydoedit(){
        ...
        $id                          = I('post.id','','strip_tags');
        if(!empty($_POST['end'])){
            $data['register_time']          = I('post.end','','strip_tags');
        }
        // dump($row);
        // dump($data);
        // dump($data);exit;
